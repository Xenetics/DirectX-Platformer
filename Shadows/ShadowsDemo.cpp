  ...
  mLightRotationAngle(0.0f)
{
	...
	SetCursorPos(mClientWidth * 0.5f, mClientHeight * 0.5f);
	...
	mSceneBounds.Radius = sqrtf(10.0f*10.0f + 15.0f*15.0f);//NEEDS TO BE FIXED!!!!!!!!!!!!!!!!!!!!!****************************** (only for shadows)

	XMMATRIX gridScale = XMMatrixScaling(3.0f, 1.0f, 3.0f);
	XMMATRIX gridOffset = XMMatrixTranslation(0.0f, 0.5f, 10.0f);
	...
	XMStoreFloat4x4(&mCylWorld[9], XMMatrixTranslation(+5.0f, -2001.5f, -10.0f + 9*5.0f));

	XMStoreFloat4x4(&mSphereWorld[9], XMMatrixTranslation(+5.0f, -2003.5f, -10.0f + 9*5.0f));
	...
	mGroundBoxs[0] = Box(XMFLOAT3(5.0, 1.0, 30.0), XMFLOAT3(0.0, 0.0, 0.0));
	mGroundBoxs[1] = Box(XMFLOAT3(10.0, 15.0, 20.0), XMFLOAT3(-8.0, 2.0, 20.0));
	mGroundBoxs[2] = Box(XMFLOAT3(0.5, 8.0, 20.0), XMFLOAT3(-14.0, 15.0, 40.0));

	mWallBoxs[0] = Box(XMFLOAT3(0.5, 8.0, 20.0), XMFLOAT3(2.0, 1000.0, 25.0));
}
...
bool ShadowsApp::Init()
{
	...
	HR(D3DX11CreateShaderResourceViewFromFile(md3dDevice, L"Textures/floor.dds", 0, 0, &mStoneTexSRV, 0 ));

	HR(D3DX11CreateShaderResourceViewFromFile(md3dDevice, L"Textures/bricks.dds", 0, 0, &mBrickTexSRV, 0 ));

	HR(D3DX11CreateShaderResourceViewFromFile(md3dDevice, L"Textures/floor_nmap.dds", 0, 0, &mStoneNormalTexSRV, 0 ));

	HR(D3DX11CreateShaderResourceViewFromFile(md3dDevice, L"Textures/bricks_nmap.dds", 0, 0, &mBrickNormalTexSRV, 0 ));
	...
	XMMATRIX skullOffset = XMMatrixTranslation(-10.0f, 10.0f, 56.0f);
	XMStoreFloat3(&mSkullBBox.Center, XMVectorAdd(XMLoadFloat3(&mSkullBBox.Center), XMLoadFloat3(&XMFLOAT3(-10.0f, 10.0f, 56.0f))));
	XMStoreFloat3(&mSkullBBox.Extents, XMVectorMultiply(XMLoadFloat3(&mSkullBBox.Extents), XMVectorReplicate(0.5f)));
...
void ShadowsApp::OnResize()
{
	...
	mPlayer.SetLens(0.25f*MathHelper::Pi, AspectRatio(), 0.1f, 1000.0f);
}
...
void ShadowsApp::UpdateScene(float dt)
{
	...
	XNA::AxisAlignedBox playerBBox = mPlayer.GetBoundingBox();

	//reseting the player
	if ((GetAsyncKeyState('R') & 0x8000) || playerPos.y < -500 || XNA::IntersectAxisAlignedBoxAxisAlignedBox(&playerBBox, &mSkullBBox) )
	{
		mPlayer.SetPosition(0.0f, 8.0f, -12.0f);
		mPlayer.vel.y = 0;
		mPlayer.vel.x = 0;
	...
	for (int i = 0; i < (sizeof(mGroundBoxs) / sizeof(mGroundBoxs[0])); i++)
	{
		XMFLOAT3 boxPos = mGroundBoxs[i].GetPosition();
		//checks if the player is alighned with the box
		if ((playerPos.y - (playerBBox.Extents.y) * 1.95f ) >(boxPos.y + mGroundBoxs[i].GetBoundingBox().Extents.y))
		{
			if (mPlayer.isCollidingFloor == false)
				mPlayer.isCollidingFloor = XNA::IntersectAxisAlignedBoxAxisAlignedBox(&playerBBox, &mGroundBoxs[i].GetBoundingBox());
		}
		else if ((playerPos.x + playerBBox.Extents.x) < (boxPos.x + mGroundBoxs[i].GetBoundingBox().Extents.x) && (playerPos.x - playerBBox.Extents.x) >(boxPos.x - mGroundBoxs[i].GetBoundingBox().Extents.x))
		{
			//OutputDebugString(L"In x Bounds \n");
			if (mPlayer.isCollidingWall == false)
			{
				mPlayer.isCollidingWall = XNA::IntersectAxisAlignedBoxAxisAlignedBox(&playerBBox, &mGroundBoxs[i].GetBoundingBox());
				mPlayer.setWallDir('x'); //or z for walls in the other direction.
			}
		}
		else if ((playerPos.z + playerBBox.Extents.z) < (boxPos.z + mGroundBoxs[i].GetBoundingBox().Extents.z) && (playerPos.z - playerBBox.Extents.z) >(boxPos.z - mGroundBoxs[i].GetBoundingBox().Extents.z))
		{
			//OutputDebugString(L"In z Bounds \n");
			if (mPlayer.isCollidingWall == false)
			{
				mPlayer.isCollidingWall = XNA::IntersectAxisAlignedBoxAxisAlignedBox(&playerBBox, &mGroundBoxs[i].GetBoundingBox());
				mPlayer.setWallDir('z'); //or z for walls in the other direction.
			}
		}
		//else
	...
	for (int i = 0; i < (sizeof(mWallBoxs) / sizeof(mWallBoxs[0])); i++)
	{
		if (mPlayer.isCollidingWall == false)
		{
			mPlayer.isCollidingWall = XNA::IntersectAxisAlignedBoxAxisAlignedBox(&playerBBox, &mWallBoxs[i].GetBoundingBox());
			mPlayer.setWallDir('x'); //or z for walls in the other direction.
		}
...
void ShadowsApp::DrawScene()
{
	...
	*/
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		for (int i = 0; i < (sizeof(mGroundBoxs) / sizeof(mGroundBoxs[0])); i++)
		{
			world = XMLoadFloat4x4(&mGroundBoxs[i].GetWorldMatrix());
			worldInvTranspose = MathHelper::InverseTranspose(world);
			worldViewProj = world*view*proj;\
			...
			activeTech->GetPassByIndex(p)->Apply(0, md3dImmediateContext);
			md3dImmediateContext->DrawIndexed(mGroundBoxs[i].GetIndexCount(), 0, 0);
		}
		...
		for (int i = 0; i < (sizeof(mWallBoxs) / sizeof(mWallBoxs[0])); i++)
		{
			world = XMLoadFloat4x4(&mWallBoxs[i].GetWorldMatrix());
			worldInvTranspose = MathHelper::InverseTranspose(world);
			worldViewProj = world*view*proj;
			...
			activeTech->GetPassByIndex(p)->Apply(0, md3dImmediateContext);
			md3dImmediateContext->DrawIndexed(mWallBoxs[i].GetIndexCount(), 0, 0);
		}
	}
...
void ShadowsApp::OnMouseMove(WPARAM btnState, int x, int y)
{
	...
	mLastMousePos.x = x;
	mLastMousePos.y = y;
	if ( x > mClientWidth * 0.9f || x < mClientWidth * 0.1f || y > mClientHeight * 0.9f || y < mClientHeight * 0.1f)
	{
		SetPhysicalCursorPos(mClientWidth * 0.5f, mClientHeight * 0.5f);
		POINT mMouse;
		GetPhysicalCursorPos(&mMouse);
...
void ShadowsApp::BuildShapeGeometryBuffers()
{
	...
	mGroundBBoxs[0].Center = XMFLOAT3(0.0, 0.0, 0.0);
	mGroundBBoxs[0].Extents = XMFLOAT3(0.5, 0.5, 0.5);
	...
	XMStoreFloat3(&mGroundBBoxs[1].Center, 0.5f*(vMin + vMax));
	XMStoreFloat3(&mGroundBBoxs[1].Extents, 0.5f*(vMax - vMin));
	...
	*/
	...
	for (int i = 0; i < (sizeof(mGroundBoxs) / sizeof(mGroundBoxs[0])); i++)
	{
		totalVertexCount += mGroundBoxs[i].GetVertexCount();
	}

	UINT k = 0;
	std::vector<Vertex::PosNormalTexTan> vertices(totalVertexCount);
	for (int j = 0; j < (sizeof(mGroundBoxs) / sizeof(mGroundBoxs[0])); j++)
	{
		for (size_t i = 0; i < mGroundBoxs[j].GetMeshData().Vertices.size(); ++i, ++k)
		{
			vertices[k].Pos = mGroundBoxs[j].GetMeshData().Vertices[i].Position;
			vertices[k].Normal = mGroundBoxs[j].GetMeshData().Vertices[i].Normal;
			vertices[k].Tex = mGroundBoxs[j].GetMeshData().Vertices[i].TexC;
			vertices[k].TangentU = mGroundBoxs[j].GetMeshData().Vertices[i].TangentU;
		}
	}
	//*/
	...
	*/
	...
	for (int i = 0; i < (sizeof(mGroundBoxs) / sizeof(mGroundBoxs[0])); i++)
	{
		totalIndexCount += mGroundBoxs[i].GetIndexCount();
	}
	...
	for (int j = 0; j < (sizeof(mGroundBoxs) / sizeof(mGroundBoxs[0])); j++)
	{
		for (size_t q = 0; q < mGroundBoxs[j].GetMeshData().Indices.size(); ++q, k++)
		{
			UINT temp = mGroundBoxs[j].GetMeshData().Indices[q];
			indices[k] = temp;
		}
	}
	//*/
...
void ShadowsApp::BuildSkullGeometryBuffers()
{
	...
	XMStoreFloat3(&mSkullBBox.Center, 0.5f*(vMin + vMax));
	XMStoreFloat3(&mSkullBBox.Extents, 0.5f*(vMax - vMin));
...
bool BoundingBoxColisionCheck(XMFLOAT3 b1Min, XMFLOAT3 b1Max, XMFLOAT3 b2Min, XMFLOAT3 b2Max)
{
	if (b1Min.x < b1Max.x && b1Max.x > b2Min.x &&
...
XMFLOAT4X4 SimpleBoxColisionCheck(XMFLOAT3 b1Min, XMFLOAT3 b1Max, XMFLOAT3 b2Min, XMFLOAT3 b2Max)
{
	XMFLOAT4X4 ret;
